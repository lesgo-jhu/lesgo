  ! When to start dynamic Ds calculations !Eshwan
!!
...
!---------------------------------------------------  
  ...
  !character(*), parameter :: OUTPUT_PATH = path 
  character(*), parameter :: OUTPUT_PATH = '/scratch0/anand-fs1/Eshwan_les/sim71e/'
  !character(*), parameter :: OUTPUT_PATH = '/datascope/gnana_esms/eshwan/sim49d/'
  character(*), parameter :: checkpoint_file = path // 'output/vel.out'
  character(*), parameter :: checkpoint_tavg_file = path // 'output/tavg.out'
  !$if($OUTPUT_EXTRA) - Eshwan 
  character(*), parameter :: checkpoint_tavg_sgs_file = output_path // 'output/tavg_sgs.out'
  !$endif - Eshwan
  character(*), parameter :: checkpoint_spectra_file = output_path // 'output/spectra.out'
...
!---------------------------------------------------
  ...
  integer :: nproc = 8 !--this must be 1 if no MPI
...
!---------------------------------------------------  
  ...
  real(rprec) :: L_x = 1.666667_rprec, L_y=1.66667_rprec, L_z=1.0_rprec

  !Mixed layer depth (dimensional)
  real(rprec) :: ml_depth = 40._rprec
...
!---------------------------------------------------   
  ...
  !Eddy viscosity
  integer :: sgs_model=3, wall_damp_exp=2
  !Eddy diffusivity
  integer :: scalars_sgs_model=5
  ...
  ! When to start scalar transport !Eshwan
  integer :: scal_init = 1
  ...
  real(rprec) :: u_star = 6.1e-3_rprec

  ! Prandtl number for scalar transport - Eshwan
  real(rprec) :: Pr_turb = 0.4_rprec   !turbulent Prandtl number
  real(rprec) :: Pr_lam = 13._rprec     !laminar Prandtl number

  ! Schmidt number for salinity - Eshwan
  real(rprec) :: Sc_turb = 0.6_rprec    !turbulent Schmidt number
  real(rprec) :: Sc_lam = 2040._rprec     !laminar Schmidt number


  ! von Karman constant     
  real(rprec) :: vonk = 0.4_rprec
 
  ! Imposed velocity at top
  real(rprec) :: u_b = 1._rprec 
 
  ! Coriolis stuff
  ! coriol=non-dim coriolis parameter,
  ! Coriolis parameter is non-dimensionalized by z_i/u_star
  ! ug=horiz geostrophic vel, vg=transverse geostrophic vel
  logical :: coriolis_forcing = .false. 
  real(rprec) :: coriol = 1.4754_rprec, ug=1.0_rprec, vg=0.0_rprec
...
!---------------------------------------------------  
  ...
  logical :: inilag = .true.
  ! initlag = true to initialize ds, I_LM & IMM !Eshwan
  logical :: inilag_scalar = .true.

  ! initt = true to read from a file; false to create analytically
  logical :: initt = .true.
   
  ! inits = true to read from a file; false to create analytically
  logical :: inits = .true.
  ! lbc: lower boundary condition:  0 - stress free, 1 - wall, 2 - prescribed 
