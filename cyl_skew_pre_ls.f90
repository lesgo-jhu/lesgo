...
!***************************************************************
  use cyl_skew_pre_base_ls, only : calc_phi
                 ...
                 do j=1,ny
                    do i=1,nx_proc
                       ...
                       if(gcs_t(i,j,k)%phi > 0._rprec) then
                          call point_location(nt,ng,nc,nb,i,j,k)
                          call set_level_set_dist(nt,ng,nc,nb,i,j,k)
                       endif
...
!**********************************************************************
subroutine point_location(nt,ng,nc,nb,i,j,k)
!**********************************************************************
!
! This subroutine computes the location of the point with respect to the
! branch in question. Really it sets the global flags:
!
!   btw_planes
!   in_cir
!   in_cyl 
...
!   below_cyl
!
! which are subsequently used for computing the level set distance to
! the surface.
!
  use types, only : rprec
  ...
  elseif(gcs_t(i,j,k)%xyz(3) < bplane) then
     ...
     write(*,*) 'Error in point_location: cannot be anywhere else'
     stop
  endif
...
!**********************************************************************
subroutine set_level_set_dist(nt,ng,nc,nb,i,j,k)
!**********************************************************************
!
! Set the level set (minimum, signed) distance to the surface of the
! branch for the given point. This gets stored as phi and is eventually
! written to the phi.out files read by lesgo.
!
  use types, only : rprec
  ...
  use cyl_skew_pre_base_ls, only : in_cyl_bottom, in_cyl, in_bottom_surf
  implicit none
  ...
  ! Set the sign based on whether the points are inside a surface or not
  if(in_cyl .or. in_bottom_surf) gcs_t(i,j,k)%phi = -dabs(gcs_t(i,j,k)%phi)

  !  Nullify pointers
  nullify(br_t_p, a,b,bplane,tplane,bot,top,skw_angle,skw_axis,brindx_p, clindx_p)

  return
end subroutine set_level_set_dist
  ...
  do k=lbz,nz_tot
     do j=1,ny
        ...
        do i=1,nx_proc
           ...
           call associated_generation( zcell_bot, gen_cell_bot )
           call associated_generation( zcell_top, gen_cell_top )
...
!**********************************************************************
subroutine associated_generation(z, gen_id)
!**********************************************************************
!
...
!
  ...
  return
end subroutine associated_generation
...
subroutine filter_indicator_function(xyz, id_gen, delta, chi, brindx)
  ...
  do nt=1, ntree
     ...
     do nc = 1, ncluster_p
        ...
        !  Filter over cluster
        call filter_cluster_chi(xyz, cl_t_p(nc), delta, chi, brdist, brindx)
...
!**********************************************************************
subroutine filter_cluster_chi(xyz, cl_t, delta, chi, brdist, brindx)
  !**********************************************************************
  ...
  call weighted_cluster_integration(a_p, b_p, angle2_p, cpnt_t, lpnt_t, nbranch_p, delta, chi_int)
...
end subroutine filter_cluster_chi

!**********************************************************************
subroutine weighted_cluster_integration(a, b, angle, cpnt_t, lpnt_t, nbranch, delta, chi)
!**********************************************************************
  use types, only : rprec
  ...
  return
end subroutine weighted_cluster_integration
...
contains
  ...
  subroutine write_output_local()
    !*********************************************************************
    !
    ! This subroutine writes Tecplot out data as it is distributed among
    ! the processors.
    !
    use mpi
    use types, only : rprec
    ...
    real(rprec), allocatable, dimension(:,:) :: phi_sbuf, phi_rbuf, chi_sbuf, chi_rbuf
    real(rprec), allocatable, dimension(:) :: x, y, z

    left = modulo( global_rank_csp - 1, nproc_csp )
    right = modulo( global_rank_csp + 1, nproc_csp )

    write(*,*) 'rank, left, right : ', global_rank_csp, left, right

    sendcnt = ny * nz_tot

    allocate( phi(nx_proc+1,ny,nz_tot) )
    allocate( brindx(nx_proc+1,ny,nz_tot) )
    allocate( clindx(nx_proc+1,ny,nz_tot) )
    allocate( chi(nx_proc+1,ny,nz_tot) )
    allocate(x(nx_proc+1),y(ny+1),z(nz_tot))
    ...
    phi(1:nx_proc,:,:) = gcs_t(:,:,1:)%phi
    chi(1:nx_proc,:,:) = gcs_t(:,:,1:)%chi
    brindx(1:nx_proc,:,:) = gcs_t(:,:,1:)%brindx
    clindx(1:nx_proc,:,:) = gcs_t(:,:,1:)%clindx
    x(1:nx_proc) = gcs_t(:,1,1)%xyz(1)
    y(1:ny) = gcs_t(1,:,1)%xyz(2)
    z = gcs_t(1,1,1:)%xyz(3)

    ! Allocate buffer arrays
    allocate( phi_sbuf(ny,nz_tot), phi_rbuf(ny,nz_tot) )
    allocate( chi_sbuf(ny,nz_tot), chi_rbuf(ny,nz_tot) )
    allocate( brindx_sbuf(ny,nz_tot), brindx_rbuf(ny,nz_tot) )
    allocate( clindx_sbuf(ny,nz_tot), clindx_rbuf(ny,nz_tot) )

    phi_sbuf = phi(1,:,:)
    chi_sbuf = chi(1,:,:)
    brindx_sbuf = brindx(1,:,:)
    clindx_sbuf = clindx(1,:,:)

    if( modulo(global_rank_csp,2) == 0 ) then

       call mpi_send( phi_sbuf, sendcnt, MPI_RPREC, left, 1, &
            MPI_COMM_WORLD, ierr)
       call mpi_send( chi_sbuf, sendcnt, MPI_RPREC, left, 2, &
            MPI_COMM_WORLD, ierr)
       call mpi_send( brindx_sbuf, sendcnt, MPI_INTEGER, left, 3, &
            MPI_COMM_WORLD, ierr)
       call mpi_send( clindx_sbuf, sendcnt, MPI_INTEGER, left, 4, &
            MPI_COMM_WORLD, ierr)

       call mpi_recv( phi_rbuf, sendcnt, MPI_RPREC, right, 5, &
            MPI_COMM_WORLD, status, ierr)
       call mpi_recv( chi_rbuf, sendcnt, MPI_RPREC, right, 6, &
            MPI_COMM_WORLD, status, ierr)
       call mpi_recv( brindx_rbuf, sendcnt, MPI_INTEGER, right, 7, &
            MPI_COMM_WORLD, status, ierr)
       call mpi_recv( clindx_rbuf, sendcnt, MPI_INTEGER, right, 8, &
            MPI_COMM_WORLD, status, ierr)

    else

       call mpi_recv( phi_rbuf, sendcnt, MPI_RPREC, right, 1, &
            MPI_COMM_WORLD, status, ierr)
       call mpi_recv( chi_rbuf, sendcnt, MPI_RPREC, right, 2, &
            MPI_COMM_WORLD, status, ierr)
       call mpi_recv( brindx_rbuf, sendcnt, MPI_INTEGER, right, 3, &
            MPI_COMM_WORLD, status, ierr)
       call mpi_recv( clindx_rbuf, sendcnt, MPI_INTEGER, right, 4, &
            MPI_COMM_WORLD, status, ierr)

       call mpi_send( phi_sbuf, sendcnt, MPI_RPREC, left, 5, &
            MPI_COMM_WORLD, ierr)
       call mpi_send( chi_sbuf, sendcnt, MPI_RPREC, left, 6, &
            MPI_COMM_WORLD, ierr)
       call mpi_send( brindx_sbuf, sendcnt, MPI_INTEGER, left, 7, &
            MPI_COMM_WORLD, ierr)
       call mpi_send( clindx_sbuf, sendcnt, MPI_INTEGER, left, 8, &
            MPI_COMM_WORLD, ierr)

    endif

    ! call mpi_sendrecv( phi_sbuf, sendcnt, MPI_RPREC, left, 1, &
    !      phi_rbuf, sendcnt, MPI_RPREC, right, 1, MPI_COMM_WORLD, status, ierr)
    ! call mpi_sendrecv( chi_sbuf, sendcnt, MPI_RPREC, left, 2, &
    !      chi_rbuf, sendcnt, MPI_RPREC, right, 2, MPI_COMM_WORLD, status, ierr)
    ! call mpi_sendrecv( brindx_sbuf, sendcnt, MPI_INTEGER, left, 3, &
    !      brindx_rbuf, sendcnt, MPI_INTEGER, right, 3, MPI_COMM_WORLD, status, ierr)
    ! call mpi_sendrecv( clindx_sbuf, sendcnt, MPI_INTEGER, left, 4, &
    !      clindx_rbuf, sendcnt, MPI_INTEGER, right, 4, MPI_COMM_WORLD, status, ierr)

    phi(nx_proc+1,:,:) = phi_rbuf
    chi(nx_proc+1,:,:) = chi_rbuf
    brindx(nx_proc+1,:,:) = brindx_rbuf
    clindx(nx_proc+1,:,:) = clindx_rbuf
    !  Set buffer x and y points
    x(nx_proc+1) = x(nx_proc)+dx
    y(ny+1) = y(ny)+dy
    ...
    call write_tecplot_header_ND(fname, 'rewind', 7, &
         ...
         numtostr(global_rank_csp,6), 2)

    call write_real_data_3D( fname, 'append', 'formatted', 4, Nx_proc+1, Ny, Nz_tot,&
         (/phi, real(brindx,rprec), real(clindx,rprec), chi /),&
         2, x, y, z)

    deallocate(phi,phi_sbuf,phi_rbuf)
    deallocate(brindx,brindx_sbuf,brindx_rbuf)
    deallocate(clindx,clindx_sbuf,clindx_rbuf)
    deallocate(chi,chi_sbuf,chi_rbuf)
  ...
  subroutine write_output_lesgo()
    ...
    if( global_rank_csp > 0 ) then
       ...
       do n=1,nproc_csp-1
          ...
          ! Recieve stride from proc n
          call mpi_recv( istart, 1, MPI_INTEGER, n, 6, &
               MPI_COMM_WORLD, status, ierr)
          ! Recieve nx_proc from proc n
          call mpi_recv( iend, 1, MPI_INTEGER, n, 7, &
               MPI_COMM_WORLD, status, ierr)
    ...
    return
  end subroutine write_output_lesgo
