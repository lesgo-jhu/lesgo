...
use sgs_param,only:cs2_clips !Eshwan
...
!        Q33 = w_bar*w_bar
    ...
    ! Count how often Cs is clipped
        do i=1,nx
        do j=1,ny
            if (real(Cs_opt2(i,j,jz)).lt.real(zero)) then 
               count_clip = count_clip + 1
               cs2_clips(i,j,jz) = 1._rprec
            end if
    ...
    ! Clip, if necessary
        ...
        !print*, 'jt, jz, Cs_opt2(25,25,jz)',jt, jz, Cs_opt2(25,25,jz)
...
! Share new data between overlapping nodes
    $if ($MPI)
        ...
        call mpi_sync_real_array( cs2_clips, 0, MPI_SYNC_DOWNUP )  
    $endif   
