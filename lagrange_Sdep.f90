!!  Copyright (C) 2009-2014  Johns Hopkins University
!!
...
do jz = 1,nz
   ...
   if (inilag) then
     ...
     print *,'F_MM, F_LM, F_NN and F_QN initialized' 
     F_MM (:,:,jz) = MM
   ...
   if (inflow) then
     ...
     LM  (istart:iend, 1:ny)    = 0._rprec
     MM  (istart:iend, 1:ny)    = 0._rprec
     F_LM(istart:iend, 1:ny,jz) = 0._rprec ! gives Tn=zero first step, Tn==> Tn large, dumfac small, epsi=1 (instantaneous)
     F_MM(istart:iend, 1:ny,jz) = 0._rprec
     QN  (istart:iend, 1:ny)    = 0._rprec
   ...
   Cs_opt2_2d(ld,:)   = 0._rprec
   Cs_opt2_2d(ld-1,:) = 0._rprec
   Cs_opt2_2d(:,:)    = max( 0._rprec, Cs_opt2_2d(:,:) )
  ...
  Cs_opt2_4d(ld,:)   = 0._rprec
  Cs_opt2_4d(ld-1,:) = 0._rprec
  Cs_opt2_4d(:,:)    = max(0._rprec,Cs_opt2_4d(:,:))

  Beta(:,:,jz)=(Cs_opt2_4d(:,:)/Cs_opt2_2d(:,:) + zero)
