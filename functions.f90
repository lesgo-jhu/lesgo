...
!**********************************************************************
function interp_eddy_to_uv_grid(var, lbz) result(var_uv) !Eshwan
!**********************************************************************
!  This function interpolates the array var, which is a modelled sgs
!  parameter, onto the uv-grid variable var_uv using linear interpolation.
!  In var, all points are on the w-grid except for the one at k-1 on 
!  coord 0, which is on the uv-grd. It is important to note that message 
!  passing is required for MPI cases and  all processors must call this 
!  routine. If this subroutine is call from a 
!
!  NOTE: It is assumed that the size of var and var_uv are the same as the
!  coord (processor) domain and that k=nz-1 (k=0) and k=1 (k=nz) are overlap
!  nodes - no interpolation is performed for k=0 and k=nz
...
character (*), parameter :: sub_name = mod_name // '.interp_eddy_to_uv_grid'

sx=size(var,1)
sy=size(var,2)
ubz=ubound(var,3)

if( ubz .ne. nz ) call error( 'interp_eddy_to_uv_grid', 'Input array must lbz:nz z dimensions.')
...
do k=jz_min,ubz-1
   do j=1,sy
      do i=1,sx
         var_uv(i,j,k) = 0.5_rprec*(var(i,j,k)+var(i,j,k+1))
      end do
   end do
end do
...
end function interp_eddy_to_uv_grid
...
!**********************************************************************
 function linreg(x,y,n) result(reg_coeffs)
!**********************************************************************
!  
...
integer, intent(in)  :: n
real(rprec) :: sumx, sumx2, sumxy, sumy, sumy2, m, b
integer:: i
...
!write(*,*) 'Slope m=', m, reg_coeffs(1), coord
!write(*,*) 'y-intercept b=', b, reg_coeffs(2), coord
