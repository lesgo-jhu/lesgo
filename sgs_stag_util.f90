...
public sgs_stag, rtnewt, calc_Sij
...
$if ($MPI)
use mpi_defs, only : mpi_sync_real_array, MPI_SYNC_DOWN, MPI_SYNC_UP
$endif
...
!            S33(jx,jy,1)=wz         
    ...
    case (2) ! Imposed shear stress !Eshwan
    !dudz and dvdz are stored on uvp nodes. dudz(:,:,1)=dudz(:,:,2) and dvdz(:,:,1)=dvdz(:,:,2)
        do jy=1,ny
        do jx=1,nx              
            ! these values stored on uvp-nodes
            S11(jx,jy,1)=dudx(jx,jy,1)         
...
!    call mpi_sync_real_array( dudz, 0, MPI_SYNC_UP ) !-Eshwan
!    call mpi_sync_real_array( dvdz, 0, MPI_SYNC_UP ) !-Eshwan
$endif
...
!-----------------------------------------------------------------------
real(kind=rprec) function rtnewt(A, jz, jt)
!-----------------------------------------------------------------------
use types,only:rprec
...
!write(6,*) 'using beta=1 at jz= , jt =', jz, jt
end function rtnewt
!-----------------------------------------------------------------------
