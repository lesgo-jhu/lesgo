...
do ip = ipmin, ipmax
  ...
  if (MPI_split) then
    ...
    !0 = ip * (nz - 1) / nproc       !--0 level (local)
    !ubz = 0 + (nz - 1) / nproc + 1  !--nz level (local)
...
!     !0 = ip * (nz - 1) / nproc + 1   !--1 level (local)
!     !ubz = 0 + (nz - 1) / nproc - 1  !--nz-1 level (local)
! 
...
!     close (1)
    ...
    if (do_write_ascii) then  !--ascii-files for checking
      ...
      !0 = ip * (nz - 1) / nproc       !--0-level
      !ubz = 0 + (nz - 1) / nproc + 1  !--nz-level
      ...
      !0 = ip * (nz - 1) / nproc + 1      !--1 level
      !ubz = 0 + (nz - 1) / nproc - 1  !--nz-1 level
